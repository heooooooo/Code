...
class Point:
    ...
    def drawPoint(self):
        pygame.draw.circle(game_window, white, convert((self.x, self.y)), 2.5 * scale/2.5)
...
while start:
    ...
    for i in range (8):
        ...
        fov = distance
        projection = np.array([[fov, 0, 0],
                            [0, fov, 0]])

        project2d = projection @ rotated
        points[i].x, points[i].y = project2d[0], project2d[1]
        points[i].drawPoint()
